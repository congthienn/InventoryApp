...
export class OrderListComponent implements OnInit {
  ...
  private updateColumnDefs() {
    this.columnDefs  =  [ 
      ...
      { field: 'branchRequest', headerName: "CHI NHÁNH", resizable:true },
...
function fragRenderer(params: ICellRendererParams) {
  ...
  return `<button type="button" class="btn ${statusClass} btn-sm-custom">${params.value == 0 ? "Chờ phê duyệt" : "Đang giao"}</button>`;
}